...
BYTE					g_aButtonStateDouble[NUM_BUTTON_MAX];	// マウスのオンオフ
...
void UpdateMouse(void)
{
	...
	if (SUCCEEDED(g_pDevMouse->GetDeviceState(sizeof(DIMOUSESTATE2), &g_mouse.mouseState)))
	{
		for (nCntButton = 0; nCntButton < NUM_BUTTON_MAX; nCntButton++)
		{
			...
			g_aButtonStateDouble[nCntButton] = (g_aButtonState[nCntButton] ^ g_mouse.mouseState.rgbButtons[nCntButton]);
...
// マウスのダブル処理
// ------------------------------------------------------------------------------------------
bool GetMousePressDouble(int nButton)
{
	return (g_aButtonStateDouble[nButton] & 0x80) ? true : false;
}
