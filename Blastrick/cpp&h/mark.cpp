...
#define MARKBOMCHAN_ANIMCOUNT (1)
#define MARKBOMCHAN_HORIZONANIM (1.0f/MARKBOMCHAN_ANIMCOUNT)
#define MARKHOSSAN_ANIMCOUNT (1)
...
void CMark::Init(void)
{
	for (int nCnt = 0; nCnt < MAX_MARK; nCnt++)
	{
		...
		m_aScene_Two[nCnt] = CScene_TWO::Create(
			...
			D3DVECTOR3_ZERO,																// 位置
			{ MARK_SIZE,MARK_SIZE },														// サイズ
			ACTOR_MARK,																		// アクター
			0.0f,																			// 角度
			D3DXCOLOR(rand() % 10 / 10.0f, rand() % 10 / 10.0f, rand() % 10 / 10.0f, 1.0f)	// 色
		);
		// テクスチャー情報を設定
...
HRESULT CMark::Load(void)
{
	...
	// テクスチャー情報代入
	char TexData[MAX_MARK][72] =
	{
		{"data/TEXTURE/ps/magic.png"},	// タイトル用の背景
		{"data/TEXTURE/ps/magic.png"},		// 選択画面用の背景
	};
	// 読み込み
	for (int nCnt = 0; nCnt < MAX_MARK; nCnt++)
	{
