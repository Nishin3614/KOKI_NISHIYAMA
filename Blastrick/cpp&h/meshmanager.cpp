// ----------------------------------------
//
// メッシュマネージャー処理の説明[meshmanager.cpp]
...
//
// ----------------------------------------
#define MESHMANAGER_ROTMOVE (0.01f)		// 角度の移動量
#define MESHMANAGER_SIZE (100.0f)			// サイズ
#define MESHMANAGER_TEX "png"				// テクスチャー
#define MESHMANAGER_ANGLE (D3DX_PI / 10)	// 湾曲の角度
...
//
// ----------------------------------------
LPDIRECT3DTEXTURE9 CMeshmanager::m_pTex = NULL;

// ----------------------------------------
// コンストラクタ処理
// ----------------------------------------
CMeshmanager::CMeshmanager() : CScene(ACTOR_MESHMANAGER, LAYER_3DOBJECT)
{
	/* 変数の初期化 */
	// 回転量
	m_pVtxBuff = NULL;
...
// コンストラクタ処理
// ----------------------------------------
CMeshmanager::CMeshmanager(ACTOR actor) : CScene(actor, LAYER_3DOBJECT)
{
	/* 変数の初期化 */
	// 回転量
	m_pVtxBuff = NULL;
...
// 読み込み処理
// ----------------------------------------
HRESULT CMeshmanager::Load(void)
{
	// デバイスの取得
	LPDIRECT3DDEVICE9 pDevice =		
		CManager::GetRenderer()->GetDevice();
	// テクスチャー設定
	D3DXCreateTextureFromFile(pDevice, MESHMANAGER_TEX, &m_pTex);
	return S_OK;
}
...
// 作成処理
// ----------------------------------------
CMeshmanager * CMeshmanager::Create(
	D3DXVECTOR3 pos,
	D3DXVECTOR3 size,
	D3DXVECTOR3 rot,
	int nWidth,
...
{
	// 変数宣言
	CMeshmanager * pMeshmanager;		// シーン2Dクラス
	// メモリの生成(初め->基本クラス,後->派生クラス)
	pMeshmanager = new CMeshmanager();
	// 位置情報
	pMeshmanager->m_pos = pos;
	// サイズ情報
	pMeshmanager->m_size = size;
	// 回転情報
	pMeshmanager->m_rot = rot;
	// 横ブロック
	pMeshmanager->m_nBlock_Width = nWidth;
	// 縦ブロック
	pMeshmanager->m_nBlock_Depth = nDepth;
	// 初期化処理
	pMeshmanager->Init();
	// 生成したオブジェクトを返す
	return pMeshmanager;
...
// 接している面の高さを取得
// ----------------------------------------
float CMeshmanager::GetHeight(D3DXVECTOR3 pos)
{
	// 変数宣言
	VERTEX_3D *pVtx;					// 頂点情報へのポイント
