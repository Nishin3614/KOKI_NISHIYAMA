...
//
// ------------------------------------------------------------------------------------------
//#define	JOYPADKEY_MAX			(256)	// キーの最大数
...
void UpdateJoyboard(void)
{
	...
	for (nCntPad = 0; nCntPad < MAX_GAMEPAD; nCntPad++)
	{
		...
		{
			aJoyState[nCntJoy] = g_aJoyStateGamepad[nCntPad][nCntJoy];	// キープレス情報保存
		}
		...
		if (SUCCEEDED(g_pDevJoyboard[nCntPad]->GetDeviceState(sizeof(g_aGamepadState[nCntPad]), &g_aGamepadState[nCntPad])))
		{
			...
			for (nCntJoy = 0; nCntJoy < JOYPADKEY_MAX; nCntJoy++)
			{
				...
				// トリガー情報の作成
				g_aJoyStateTriggerGamepad[nCntPad][nCntJoy] = 
					(aJoyState[nCntJoy] ^ g_aJoyStateGamepad[nCntPad][nCntJoy]) & g_aJoyStateGamepad[nCntPad][nCntJoy];

				// リリース情報の作成
				g_aJoyStateReleaseGamepad[nCntPad][nCntJoy] = 
					(aJoyState[nCntJoy] ^ g_aJoyStateGamepad[nCntPad][nCntJoy]) & !g_aJoyStateGamepad[nCntPad][nCntJoy];
